...
> NOTE: the cheat menu now includes auto-answering for questions when the game loads.
> Visit `/hacks` for extra cosmetics, skins, and boost hacks.
...
5. in the terminal (where it says `/workspaces/Gimmick (master) $`), enter the following command:
   1. `./setup.sh`
